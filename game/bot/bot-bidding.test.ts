...
import { evaluateHand, getBotBid, selectBotTrump } from './bot-bidding';
...
describe('evaluateHand', () => {
  ...
  test('total equals highCards + voids + longSuits', () => {
    ...
    expect(result.total).toBe(
      result.highCards + result.voids + result.longSuits,
    );
  });
...
describe('getBotBid — bid 10 caution', () => {
  ...
  test('easy bot is more cautious about bidding 10 than hard bot', () => {
    ...
    let easyBid10 = 0,
      hardBid10 = 0;
    for (let i = 0; i < 200; i++) {
      if (getBotBid(borderlineHand, 'easy', BID_MIN - 1) === BID_MAX)
        easyBid10++;
      if (getBotBid(borderlineHand, 'hard', BID_MIN - 1) === BID_MAX)
        hardBid10++;
    }
...
describe('selectBotTrump — hard bot minimum trump length', () => {
  test('hard bot avoids suits shorter than HARD_BOT_MIN_TRUMP_LENGTH', () => {
    ...
    const hand = [
      card('spades', 'A'),
      card('spades', 'K'), // length 2 — should be avoided
      card('hearts', 'Q'),
      card('hearts', 'J'),
      card('hearts', '10'),
      card('hearts', '9'),
      card('hearts', '8'), // length 5
      card('clubs', '2'),
    ];
  ...
  test('easy bot CAN pick a short suit (no minimum length restriction)', () => {
    ...
    expect(['spades', 'hearts', 'clubs', 'diamonds'] as Suit[]).toContain(
      trump,
    );
  });
