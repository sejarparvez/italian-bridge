...
import { advanceToNextRound, createInitialState, getWinner, isGameOver } from './engine';
import { calculateRoundScores, updateTeamScores, type RoundScore } from './scoring';
...
describe('updateTeamScores', () => {
  ...
  test('accumulates correctly over multiple rounds', () => {
    ...
    const rounds: RoundScore[][] = [
      [{ team: 'BT', bid: 7, tricks: 7, points: 7 }, { team: 'LR', bid: null, tricks: 3, points: -4 }],
      [{ team: 'BT', bid: null, tricks: 3, points: -4 }, { team: 'LR', bid: 8, tricks: 8, points: 8 }],
      [{ team: 'BT', bid: 9, tricks: 9, points: 9 }, { team: 'LR', bid: null, tricks: 1, points: -4 }],
    ];
...
describe('createInitialState', () => {
  ...
  test('all 20 dealt cards are unique (no duplicates)', () => {
    const { players } = createInitialState();
    const allIds = ['bottom', 'top', 'left', 'right'].flatMap(
      seat => players[seat as SeatPosition].hand.map(c => c.id)
    );
    expect(new Set(allIds).size).toBe(allIds.length);
  });

  test('each new game produces a different card distribution (shuffled)', () => {
    const ids1 = createInitialState().players.bottom.hand.map(c => c.id).join(',');
    const ids2 = createInitialState().players.bottom.hand.map(c => c.id).join(',');
