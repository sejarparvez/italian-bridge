...
describe('rank sorting within a suit', () => {
  test('sorts descending by value within the same suit', () => {
    const hand = [card('spades', '7'), card('spades', 'A'), card('spades', '9')];
  ...
  test('sorts all ranks correctly', () => {
    const hand = [
      card('hearts', '3'), card('hearts', 'K'), card('hearts', '7'),
      card('hearts', 'A'), card('hearts', 'J'), card('hearts', '2'),
    ];
    const result = sortHandAlternating(hand);
...
describe('only black suits', () => {
  test('spades only — sorted by rank descending', () => {
    const hand = [card('spades', '8'), card('spades', 'K'), card('spades', '2')];
    const result = sortHandAlternating(hand);
  ...
  test('spades + clubs — spades first (preferred), then clubs', () => {
    const hand = [
      card('clubs', '10'), card('clubs', '7'),
      card('spades', '9'), card('spades', 'A'),
    ];
    const result = sortHandAlternating(hand);
...
describe('only red suits', () => {
  test('hearts only — sorted by rank descending', () => {
    const hand = [card('hearts', '3'), card('hearts', 'A'), card('hearts', '8')];
    const result = sortHandAlternating(hand);
  ...
  test('hearts + diamonds — hearts first, then diamonds', () => {
    const hand = [
      card('diamonds', 'Q'), card('diamonds', '6'),
      card('hearts', '10'), card('hearts', 'A'),
    ];
    const result = sortHandAlternating(hand);
...
describe('1 red + 1 black → R B', () => {
  test('hearts + spades', () => {
    const hand = [
      card('spades', '9'), card('spades', 'A'),
      card('hearts', '7'), card('hearts', 'J'),
    ];
    const result = sortHandAlternating(hand);
    expect(colors(result)).toEqual(['R', 'R', 'B', 'B']);
    expect(suits(result).slice(0, 2).every(s => s === 'hearts')).toBe(true);
    expect(suits(result).slice(2).every(s => s === 'spades')).toBe(true);
  });
...
describe('2 red + 1 black → R B R', () => {
  test('hearts + diamonds + spades', () => {
    const hand = [
      card('hearts', 'A'), card('hearts', '9'),
      card('diamonds', '10'), card('diamonds', '3'),
      card('spades', 'J'),
    ];
    const result = sortHandAlternating(hand);
  ...
  test('hearts + diamonds + clubs', () => {
    const hand = [
      card('hearts', 'K'),
      card('diamonds', '7'), card('diamonds', '4'),
      card('clubs', 'A'), card('clubs', '9'),
    ];
    const result = sortHandAlternating(hand);
...
describe('1 red + 2 black → B R B', () => {
  test('hearts + spades + clubs', () => {
    const hand = [
      card('hearts', 'A'),
      card('spades', '10'), card('spades', '7'),
      card('clubs', '9'), card('clubs', '3'),
    ];
    const result = sortHandAlternating(hand);
...
describe('all 4 suits → R B R B', () => {
  test('alternates red/black: hearts → spades → diamonds → clubs', () => {
    const hand = [
      card('clubs', '8'), card('clubs', '3'),
      card('diamonds', '6'), card('diamonds', 'Q'),
      card('hearts', 'A'), card('hearts', '9'),
      card('spades', 'J'), card('spades', '5'),
    ];
    const result = sortHandAlternating(hand);
  ...
  test('within each suit cards are sorted highest to lowest', () => {
    const hand = [
      card('hearts', '3'), card('hearts', 'A'), card('hearts', '7'),
      card('spades', '9'), card('spades', '2'),
      card('diamonds', 'J'), card('diamonds', '5'),
      card('clubs', 'K'), card('clubs', '6'),
    ];
    const result = sortHandAlternating(hand);
...
describe('edge cases', () => {
  test('duplicate ranks within a suit', () => {
    const hand = [card('hearts', '10'), card('hearts', '10'), card('hearts', '10')];
    const result = sortHandAlternating(hand);
    expect(result).toHaveLength(3);
    expect(result.every(c => c.suit === 'hearts' && c.rank === '10')).toBe(true);
  });
  ...
  test('face cards sort correctly: A K Q J', () => {
    const hand = [card('spades', 'J'), card('spades', 'A'), card('spades', 'Q'), card('spades', 'K')];
    const result = sortHandAlternating(hand);
